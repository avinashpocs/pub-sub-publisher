...
import com.infogain.gcp.poc.repository.PNROutBoxConfirmationRepository;
...
public class PublishService {
	...
	@Autowired
	PNROutBoxConfirmationRepository pnrOutBoxConfirmationRepository;
	...
	private void publishMessage(String message, Map<String, String> attributes, String pnrOutBoxId) {
		log.info("publishing message {} to topic {}", message, topicName);

		try {
			ListenableFuture<String> future = pubSubTemplate.publish(topicName, message, attributes);
			System.out.println("Published message ID: " + future.get()+pnrOutBoxId);
			if (future.get() != null) {
				PNROutBoxConfirmationEntity pnrOutBoxConfirmationEntity = PNROutBoxConfirmationEntity.builder()
						.id(pnrOutBoxId).processStatus(true).build();
				savePNROutBoxConfirmationEntity(pnrOutBoxConfirmationEntity);
			}
	...
	private PNROutBoxConfirmationEntity savePNROutBoxConfirmationEntity(
			PNROutBoxConfirmationEntity pnrOutBoxConfirmationEntity) {
		return pnrOutBoxConfirmationRepository.save(pnrOutBoxConfirmationEntity);
	}
	...
	public String publish(PNRModel pnrModel) {
		...
		try {
			...
			publishMessage(messageJson, attributes, pnrModel.getPnrOutBoxId());
			...
			// Optional<PNROutBoxEntity> pnrOutBoxEntityOptional =
			// pnrOutBoxRepository.findById(pnrId);
	...
	public String publishFailedRecords() {
		List<PNREntity> pnrEntityList = ListUtils
				.emptyIfNull(pnrRepository.findPNREntityListByIsProcessedAndRetryCount(false, 5));
		...
		pnrEntityList.forEach(pnrEntity -> {
			Map<String, String> attributes = new HashMap<>();
			String messageJson = processMessage(pnrEntity, attributes);
			PublishMessageModel publishMessageModel = PublishMessageModel.builder().message(messageJson)
					.pnrOutBoxId(pnrEntity.getPnrOutBoxId()).attributes(attributes).build();
		...
		publishMessageModelList.forEach(publishMessageModel -> {
			publishMessage(publishMessageModel.getMessage(), publishMessageModel.getAttributes(),
					publishMessageModel.getPnrOutBoxId());
		});
		...
		pnrOutBoxEntityList.forEach(pnrOutBoxEntity -> {
			...
			if (Constants.RETRY_PNR_ID_LIST.contains(pnrOutBoxEntity.getPnrId())
					&& pnrOutBoxEntity.getRetryCount() > 2) {
				pnrOutBoxEntity.setIsProcessed(true);
			}
	...
	public String publishUnprocessedMsg() {

		List<PNROutBoxEntity> pnrOutBoxEntityList = ListUtils
				.emptyIfNull(pnrOutBoxRepository.findUnProcessedPayload(false));
		if (CollectionUtils.isEmpty(pnrOutBoxEntityList)) {
			log.info("No unprocessed PNREntity records");
			return "success";
		}
		
		pnrOutBoxEntityList.forEach(pnrOutBoxEntity -> {
			publishMessage(pnrOutBoxEntity.getPayload(), null,
					pnrOutBoxEntity.getId());
		});
